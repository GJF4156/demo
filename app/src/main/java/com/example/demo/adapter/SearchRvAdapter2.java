...
public class SearchRvAdapter2 extends RecyclerView.Adapter<SearchRvAdapter2.ViewHolder> {

    private Context mContext;
    private List<Result.DataBean> mDataBeanList;//保存数据源的列表
    private OnItemClickListener mListener;//初始化监听接口

    public SearchRvAdapter2(Context context, List<Result.DataBean> dataBeanList, OnItemClickListener listener) {
    ...
    @NonNull
    @Override
    public SearchRvAdapter2.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new SearchRvAdapter2.ViewHolder(LayoutInflater.from(mContext)
                .inflate(R.layout.speach_search_item, parent, false));
    }

    @Override
    public void onBindViewHolder(@NonNull SearchRvAdapter2.ViewHolder holder, final int position) {
        holder.name.setText(mDataBeanList.get(position).getGname());
        holder.sortDescription.setText(mDataBeanList.get(position).getGtip());
        switch ((int) mDataBeanList.get(position).getGtype()) {
                    case 0:
                        //可回收垃圾
